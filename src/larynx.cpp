...
void Larynx<ftype>::Process(ftype Pin)
{
    ...
    if (compute_powers_) {
        ...
        pmid = 0.5 * (p_(idx_now_, Eigen::placeholders::all) + p_(idx_next_, Eigen::placeholders::all)).transpose();
    ...
    // Step 1: q_ update
    q_(idx_next_, Eigen::placeholders::all) =
        q_(idx_now_, Eigen::placeholders::all).transpose() +
        dt_ * mass_matrix_inv_ * p_(idx_now_, Eigen::placeholders::all).transpose();
    ...
    rhs_.head(3) =
        ...
        (1 / dt_ * Eigen::Matrix<ftype, 3, 3>::Identity() - dissipation_matrix_left_ * mass_matrix_inv_left) *
            p_(idx_now_, Eigen::seq(0, 2)).transpose() -
        dt_ / 4 * g_sav_.head(3) *
            (g_sav_.transpose() * mass_matrix_inv_ * p_(idx_now_, Eigen::placeholders::all).transpose()) -
    ...
    // Step 5: r_ update
    r_(idx_next_) = r_(idx_now_) +
                    0.5 * dt_ * g_sav_.transpose() * mass_matrix_inv_ *
