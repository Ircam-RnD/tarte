...
template<typename ftype>
void Larynx<ftype>::RecomputeMatrices()
{
...
void Larynx<ftype>::ComputeSavVector()
{
    ...
    Enl_ += 0.25 * left_vf_.eta_stiffness_ *
            (left_vf_.stiffnesses_.diagonal().array() * elongations_.array() * elongations_.array() *
             elongations_.array() * elongations_.array())
                .sum();
    Fnl_.head(3) +=
        left_vf_.eta_stiffness_ * BodyCoverVF<ftype>::elongation_matrix_.transpose() *
        (left_vf_.stiffnesses_.diagonal().array() * elongations_.array() * elongations_.array() * elongations_.array())
            .matrix();
    // Right fold
    elongations_ = BodyCoverVF<ftype>::elongation_matrix_ * q_(idx_next_, Eigen::seq(3, 5)).transpose();

    Enl_ += 0.25 * right_vf_.eta_stiffness_ *
            (right_vf_.stiffnesses_.diagonal().array() * elongations_.array() * elongations_.array() *
             elongations_.array() * elongations_.array())
                .sum();
    Fnl_.tail(3) +=
        right_vf_.eta_stiffness_ * BodyCoverVF<ftype>::elongation_matrix_.transpose() *
        (right_vf_.stiffnesses_.diagonal().array() * elongations_.array() * elongations_.array() * elongations_.array())
            .matrix();
...
void Larynx<ftype>::Process(ftype Pin)
{
    ...
    C0_feedback_ = 1 / (Rk_ + 1 / b_resonator_) *
                   (a_resonator_ / b_resonator_ + Rk_ * Psub_(idx_next_) +
                    0.5 * effective_surfaces_Psup_.transpose() *
                        (mass_matrix_inv_left * p_(idx_now_, Eigen::seq(0, 2)).transpose() +
                         mass_matrix_inv_right * p_(idx_now_, Eigen::seq(3, 5)).transpose()));
    C1_feedback_.head(3) = 1 / (Rk_ + 1 / b_resonator_) * 0.5 * mass_matrix_inv_left * effective_surfaces_Psup_;
    C1_feedback_.tail(3) = 1 / (Rk_ + 1 / b_resonator_) * 0.5 * mass_matrix_inv_right * effective_surfaces_Psup_;
    ...
    u_woodburry_.col(1).head(3) = effective_surfaces_Psup_;
    u_woodburry_.col(1).tail(3) = effective_surfaces_Psup_;

    v_woodburry_.row(0) = mass_matrix_inv_ * g_sav_;
    v_woodburry_.row(1).head(3) = C1_feedback_;
    v_woodburry_.row(1).tail(3) = C1_feedback_;
